fbuilderjQuery=(typeof fbuilderjQuery!='undefined')?fbuilderjQuery:jQuery.noConflict();fbuilderjQuery(window).bind('pageshow',function(event){if(typeof event.originalEvent['persisted']!='undefined'&&event.originalEvent['persisted'])location.reload();});fbuilderjQuery.fbuilderjQueryGenerator=function(){(function($){if(!('fbuilder' in $))
...
{fbuilderjQuery['fbuilder']['extend_window'](fbuilderjQuery['fbuilder']['modules']['cookies']['prefix'],CF_COOKIES);}};$.fbuilder['version']='1.2.35';$.fbuilder['controls']=$.fbuilder['controls']||{};$.fbuilder['forms']=$.fbuilder['forms']||{};$.fbuilder['htmlEncode']=window['cff_esc_attr']=function(value)
...
$("#fieldlist"+opt.identifier+" .pbreak:not(.pb0)").find(".field").addClass("ignorepb");$("#fieldlist"+opt.identifier).find('[type="date"],[type="hidden"]').each(function(){$(this).rules('add',{step:false});});};var fform=function(){};$.extend(fform.prototype,{title:"Untitled Form",description:"This is my form. Please fill it out. It's awesome!",formlayout:"top_aligned",formtemplate:"",evalequations:1,evalequations_delay:0,evalequationsevent:2,loading_animation:0,animate_form:0,animation_effect:'fade',autocomplete:1,show:function(){return'<div class="fform" id="field">'+(!/^\s*$/.test(this.title)?'<h2 class="cff-form-title">'+this.title+'</h2>':'')+(!/^\s*$/.test(this.description)?'<span class="cff-form-description">'+this.description+'</span>':'')+'</div>';},after_show:function(id){if(typeof $['validator']!='undefined')
...
$(document).on('formReady','form#'+this.formId,(function(opt,fid){return function(evt,fid2){if(fid==fid2)
$.fbuilder.cpcffLoadDefaults(opt);};})(opt,this.formId));}}};$.fbuilder['forms'][opt.identifier]=ffunct;this.fBuild=ffunct;return this;};$.fbuilder.controls['ffields']=function(){};$.extend($.fbuilder.controls['ffields'].prototype,{form_identifier:"",name:"",shortlabel:"",index:-1,ftype:"",userhelp:"",audiotutorial:"",userhelpTooltip:false,csslayout:"",init:function(){},_getAttr:function(attr,raw)
...
else if((!me.showDatepicker||''==$.trim($('[id="'+me.name+'_date"]').val()))&&me.showTimepicker)return(h*3600000+m*60000)/86400000;}}
...
{var m=this,tmp;$(document).off('click','[id*="'+m.name+'"]').on('click','[id*="'+m.name+'"]',function(){m.enable_disable();});m.enable_disable();if(0<m.max&&0<m.min&&m.max<m.min){tmp=m.min;m.min=m.max;m.max=tmp;}
if(0<m.max)
